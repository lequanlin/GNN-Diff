2000## Adopted from https://github.com/rosinality/denoising-diffusion-pytorch with some minor changes.
...
class verysmall(ODEncoder2Decoder):
    def __init__(self, in_dim, input_noise_factor, latent_noise_factor):
        fold_rate = 2
        kernel_size = 2
        enc_channel_list = [2, 2, 2, 2]
        dec_channel_list = [2, 64, 64, 8]
        super(verysmall, self).__init__(in_dim, kernel_size, fold_rate, input_noise_factor, latent_noise_factor, enc_channel_list, dec_channel_list)
...
class large(ODEncoder2Decoder):
    def __init__(self, in_dim, input_noise_factor, latent_noise_factor):
        fold_rate = 5
        kernel_size = 5
        enc_channel_list = [6, 6, 6, 6]
        dec_channel_list = [6, 512, 512, 8]
        super(large, self).__init__(in_dim, kernel_size, fold_rate, input_noise_factor, latent_noise_factor, enc_channel_list, dec_channel_list)

class larger(ODEncoder2Decoder):
    def __init__(self, in_dim, input_noise_factor, latent_noise_factor):
        fold_rate = 7
        kernel_size = 7
        enc_channel_list = [6, 6, 6, 6]
        dec_channel_list = [6, 512, 512, 8]
        super(larger, self).__init__(in_dim, kernel_size, fold_rate, input_noise_factor, latent_noise_factor, enc_channel_list, dec_channel_list)

class exlarge(ODEncoder2Decoder):
    def __init__(self, in_dim, input_noise_factor, latent_noise_factor):
        fold_rate = 8
        kernel_size = 8
        enc_channel_list = [6, 6, 6, 6]
        dec_channel_list = [6, 512, 512, 8]
        super(exlarge, self).__init__(in_dim, kernel_size, fold_rate, input_noise_factor, latent_noise_factor, enc_channel_list, dec_channel_list)

class exlarger(ODEncoder2Decoder):
    def __init__(self, in_dim, input_noise_factor, latent_noise_factor):
        fold_rate = 9
        kernel_size = 9
        enc_channel_list = [6, 6, 6, 6]
        dec_channel_list = [6, 512, 512, 8]
        super(exlarger, self).__init__(in_dim, kernel_size, fold_rate, input_noise_factor, latent_noise_factor, enc_channel_list, dec_channel_list)

class huge(ODEncoder2Decoder):
    def __init__(self, in_dim, input_noise_factor, latent_noise_factor):
        fold_rate = 10
        kernel_size = 10
        enc_channel_list = [6, 6, 6, 6]
        dec_channel_list = [6, 512, 512, 8]
        super(huge, self).__init__(in_dim, kernel_size, fold_rate, input_noise_factor, latent_noise_factor, enc_channel_list, dec_channel_list)

class huger(ODEncoder2Decoder):
    def __init__(self, in_dim, input_noise_factor, latent_noise_factor):
        fold_rate = 11
        kernel_size = 11
        enc_channel_list = [6, 6, 6, 6]
        dec_channel_list = [6, 512, 512, 8]
        super(huger, self).__init__(in_dim, kernel_size, fold_rate, input_noise_factor, latent_noise_factor, enc_channel_list, dec_channel_list)

class exhuge(ODEncoder2Decoder):
    def __init__(self, in_dim, input_noise_factor, latent_noise_factor):
        fold_rate = 12
        kernel_size = 12
        enc_channel_list = [6, 6, 6, 6]
        dec_channel_list = [6, 512, 512, 8]
        super(exhuge, self).__init__(in_dim, kernel_size, fold_rate, input_noise_factor, latent_noise_factor, enc_channel_list, dec_channel_list)

class mega(ODEncoder2Decoder):
    def __init__(self, in_dim, input_noise_factor, latent_noise_factor):
        fold_rate = 13
        kernel_size = 13
        enc_channel_list = [6, 6, 6, 6]
        dec_channel_list = [6, 512, 512, 8]
        super(mega, self).__init__(in_dim, kernel_size, fold_rate, input_noise_factor, latent_noise_factor, enc_channel_list, dec_channel_list)

class exmega(ODEncoder2Decoder):
    def __init__(self, in_dim, input_noise_factor, latent_noise_factor):
        fold_rate = 15
        kernel_size = 15
        enc_channel_list = [7, 7, 7, 7]
        dec_channel_list = [7, 512, 512, 8]
        super(exmega, self).__init__(in_dim, kernel_size, fold_rate, input_noise_factor, latent_noise_factor, enc_channel_list, dec_channel_list)
