...
class AE_CNN_bottleneck(nn.Module):
    ...
    def normalize_input(self,x):
        return (x - x.mean(2).unsqueeze(1))/x.std(2).unsqueeze(1)

    def forward(self, input, time, graph_cond, cond=None, normalization = True):
        ...
        graph_info = graph_cond.repeat((input.shape[0], 1, 1)).mean(dim = 1).unsqueeze(dim = 1)
        ...
        if normalization:
            input, time_info, graph_info = self.normalize_input(input), self.normalize_input(
                time_info), self.normalize_input(graph_info)

        emb_enc1 = self.enc1(input + time_info + graph_info)
        emb_enc2 = self.enc2(emb_enc1 + time_info + graph_info)
        emb_enc3 = self.enc3(emb_enc2 + time_info + graph_info)
        emb_enc4 = self.enc4(emb_enc3 + time_info + graph_info)

        emb_dec1 = self.dec1(emb_enc4 + time_info + graph_info) + emb_enc3
        emb_dec2 = self.dec2(emb_dec1 + time_info + graph_info) + emb_enc2
        emb_dec3 = self.dec3(emb_dec2 + time_info + graph_info) + emb_enc1
        emb_dec4 = self.dec4(emb_dec3 + time_info + graph_info)
