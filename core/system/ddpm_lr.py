...
class DDPM_LR(BaseSystem):
    def __init__(self, config, **kwargs):
        super(DDPM_LR, self).__init__(config)
    ...
    def generate(self, batch, graph_cond, num=10, history=False):
    ...
    def progressive_samples_fn_simple(self, model, shape, graph_cond, device, include_x0_pred_freq=50, history=False):
    ...
    def validation_step(self, batch, batch_idx, graph_cond, **kwargs: Any):
        batch = self.pre_process(batch)
        outputs = self.generate(batch, graph_cond, 5)
        ...
        for i in range(params.shape[0]):
            ...
            f1s.append(f1)
        best_f1 = np.max(f1)
        formatted_f1s = ["{:.4f}".format(number) for number in f1s]
        print("generated models f1: {}".format(formatted_f1s))
        print("generated models mean f1: {:.4f}".format(np.mean(f1s)))
        print("generated models best f1: {:.4f}".format(best_f1))
        self.log('best_g_f1', best_f1)
        return {'best_g_f1': best_f1, 'mean_g_f1': np.mean(f1s).item()}

    def test_step(self, batch, batch_idx, graph_cond, **kwargs: Any):
        # generate 100 models
        batch = self.pre_process(batch)
        outputs = self.generate(batch, graph_cond, 100)
        params = self.post_process(outputs)
        f1s_val = []
        f1s = []
        for i in range(params.shape[0]):
            param = params[i]
            f1_val, val_loss, output_list_val = self.task_func_val(param)
            f1, test_loss, output_list = self.task_func(param)
            f1s_val.append(f1_val)
            f1s.append(f1)
        best_f1_idx = np.argmax(f1s_val)
        best_f1 = f1s[best_f1_idx]
        global_best_f1 = np.max(f1s)

        # print("generated models f1:", f1s)
        # print("generated models validation f1:", f1s_val)
        print("generated models mean f1:", np.mean(f1s))
        print("generated models median f1:", np.median(f1s))
        print("generated models best f1:", best_f1)
        print("generated models global best f1:", global_best_f1)

        self.log('best_g_f1', best_f1)
        self.log('global_best_g_f1', global_best_f1)
        self.log('mean_g_f1', np.mean(f1s).item())
        self.log('med_g_f1', np.median(f1s).item())
        return {'best_g_f1': best_f1, 'global_best_g_f1': global_best_f1, 'mean_g_f1': np.mean(f1s).item(), 'med_g_f1': np.median(f1s).item()}

    def forward(self, batch, graph_cond, **kwargs):
        batch = self.pre_process(batch)
        model = self.model
        ...
        # TODO: now only mse works for graph data
        if self.loss_type == 'kl':
            ...
            model_output = model(x_t, time, graph_cond, cond=lab)
        ...
        self.log('train_loss', loss.cpu().detach().mean().item(), on_epoch=True, prog_bar=True, logger=True)
        return loss
    ...
    def p_mean_variance(self, model, x, t, graph_cond, clip_denoised, return_pred_x0):

        # import pdb; pdb.set_trace()
        # model_output = model(x, t, cond=lab)
        model_output = model(x, t, graph_cond)
    ...
    def p_sample(self, model, x, t, graph_cond, noise_fn, clip_denoised=True, return_pred_x0=False):
        # pdb.set_trace()
        mean, _, log_var, pred_x0 = self.p_mean_variance(model, x, t, graph_cond, clip_denoised, return_pred_x0=True)
    ...
    def p_sample_loop_progressive_simple(self, model, shape, graph_cond, device, noise_fn=torch.randn,
