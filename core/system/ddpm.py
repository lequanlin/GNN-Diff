...
class DDPM(BaseSystem):
    ...
    def validation_step(self, batch, batch_idx, graph_cond, **kwargs: Any):
        ...
        outputs = self.generate(batch, graph_cond, 10)
        ...
        formatted_accs = ["{:.2f}".format(number) for number in accs]
        print("generated models accuracy: {}".format(formatted_accs))
        print("generated models mean accuracy: {:.2f}".format(np.mean(accs)))
        print("generated models best accuracy: {:.2f}".format(best_acc))
    ...
    def test_step(self, batch, batch_idx, graph_cond, **kwargs: Any):
        ...
        for i in range(params.shape[0]):
            param = params[i]
            acc_val, val_loss, output_list_val = self.task_func_val(param)
            acc, test_loss, output_list = self.task_func(param)
            accs_val.append(acc_val)
            accs.append(acc)
        best_acc_idx = np.argmax(accs_val)
        best_acc = accs[best_acc_idx]
        global_best_acc = np.max(accs)

        # print("generated models accuracy:", accs)
        # print("generated models validation accuracy:", accs_val)
        ...
        print("generated models global best accuracy:", global_best_acc)

        self.log('best_g_acc', best_acc)
        self.log('global_best_g_acc', global_best_acc)
        self.log('mean_g_acc', np.mean(accs).item())
        self.log('med_g_acc', np.median(accs).item())
        return {'best_g_acc': best_acc, 'global_best_g_acc': global_best_acc, 'mean_g_acc': np.mean(accs).item(), 'med_g_acc': np.median(accs).item()}
