...
class H2GCN(nn.Module):
    def __init__(self, in_channels, out_channels, num_hid = 64, k=2, dropout=0,
                 use_relu=True):
        ...
        self.w_embed = nn.Parameter(
            torch.zeros(size=(in_channels, num_hid)),
            requires_grad=True
        )
        self.w_classify = nn.Parameter(
            torch.zeros(size=((2 ** (self.k + 1) - 1) * num_hid, out_channels)),
            requires_grad=True
        )
    ...
    def _spspmm(sp1: torch.sparse.Tensor, sp2: torch.sparse.Tensor) -> torch.sparse.Tensor:
        ...
        indices, values = thsp.spspmm(index1, value1, index2, value2, m, n, k)
        return torch.sparse_coo_tensor(
            indices=indices,
            values=values,
            size=(m, k),
    ...
    def forward(self, x, adj):
        ...
        for i in range(self.k):
            r_last = rs[-1]
            r1 = torch.spmm(self.a1.to(device), r_last)
            # rs.append(self.act(r1))
            r2 = torch.spmm(self.a2.to(device), r_last)
